...
async function bootstrap() {
  ...
  const httpServer = require("https").createServer({
    key: fs.readFileSync("d:/server.key", 'utf8'),
    cert: fs.readFileSync("d:/fzguard.crt", 'utf8')
  /*   pfx: fs.readFileSync("d:/fzguard.pfx", 'utf8'), 
    passphrase:'Marinha2021!',
    agent:false,
...
io.on('connection', (socket) => {
	...
	socket.on('connect', () => {
	  console.log('connect', `sock=${socket.id}`);
	});
